CREATE DEFINER=`root`@`localhost` PROCEDURE `arrange_playerCards`(
	IN gID INT
)
BEGIN
    ...
    DECLARE deckID TINYINT DEFAULT getPileID('deck');
    ...
    arrange_cards: LOOP
        ...
        DO
			...
			IF epidemicCount < epidemicsToInsert THEN -- Determine the nextEpidemicIdx...
				...
				SET nextEpidemicIdx = getRandBetween(divStartIdx, divEndIdx);
			END IF;
		END WHILE;
        ...
        IF playersNeedCards THEN
			...
			ELSE
				call msg('players have their cards!');
		...
		IF playersNeedCards = FALSE THEN
            ...
            SET idx = idx + 1;
		END IF;
        -- SET lastCityKey = cKey;
    END LOOP arrange_cards;
    CLOSE card_cursor;
    
    CALL recordStartingHands(gID);
    
    -- The player with the highest city population from the initially dealt player cards goes first
    UPDATE game
    SET turnNumber = 1,
		turnRoleID =	(SELECT pileID FROM vw_playercard
						WHERE game = gID
                        AND pileID != deckID
						AND pop =	(SELECT MAX(pop)
									FROM vw_playercard
									WHERE game = gID
									AND pileID != deckID))
	WHERE gameID = gID;
